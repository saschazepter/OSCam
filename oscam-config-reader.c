...
static void tiger_rsakey_fn(const char *token, char *value, void *setting, FILE *f)
{
	struct s_reader *rdr = setting;
	if(value)
	{
		...
		else
		{
			...
			{
				fprintf(stderr, "reader tiger_rsakey parse error, %s=%s\n", token, value);
				rdr->rsa_mod_tiger_length = 0;
	...
	{
		char tmp[len * 2 + 1];
		fprintf_conf(f, "tiger_rsakey", "%s\n", cs_hexdump(0, rdr->rsa_mod_tiger, len, tmp, sizeof(tmp)));
	}
	else if(cfg.http_full_cfg)
		{ fprintf_conf(f, "tiger_rsakey", "\n"); }
}
...
static const struct config_list reader_opts[] =
{
	...
	DEF_OPT_FUNC("tiger_rsakey"                   , 0,                                    tiger_rsakey_fn),
