...
/* ===================================================================
 * LEGACY STATIC-LINK PATH (OpenSSL < 1.1.0, no dlopen)
 * Use ONLY native OpenSSL symbols:
 *   - X509V3_EXT_conf_nid()
 *   - STACK_OF(GENERAL_NAME)
 *   - sk_GENERAL_NAME_...
 *   - DIRECT GENERAL_NAME->type and ->d fields
 * =================================================================== */
	{
		/* ---- Extensions ---- */
		X509V3_CTX ctx;
...

	{
		...
		/* DNS: CN.local */
		{
			...
			GENERAL_NAME *gn = GENERAL_NAME_new();
			ASN1_IA5STRING *dns = ASN1_IA5STRING_new();
			ASN1_STRING_set(dns, buf, strlen(buf));
			gn->type = GEN_DNS;
			gn->d.ia5 = dns;
			sk_GENERAL_NAME_push(gens, gn);
		}
...
#else  /* !(OPENSSL_VERSION_NUMBER < 0x10100000L && !WITH_OPENSSL_DLOPEN) */

	/* ===================== MODERN / DLOPEN PATH (existing code) ===================== */
