...
addons="WEBIF WEBIF_LIVELOG WEBIF_JQUERY WEBIF_WIKI WITH_COMPRESS_WEBIF HAVE_DVBAPI WITH_EXTENDED_CW WITH_NEUTRINO READ_SDT_CHARSETS CS_ANTICASC WITH_DEBUG MODULE_MONITOR WITH_LB CS_CACHEEX CS_CACHEEX_AIO CW_CYCLE_CHECK LCDSUPPORT LEDSUPPORT CLOCKFIX IPV6SUPPORT WITH_ARM_NEON WITH_SIGNING"
...
libraries="WITH_SSL WITH_LIB_MINILZO WITH_LIB_AES WITH_LIB_MD5 WITH_LIB_MDC2 WITH_LIB_DES WITH_LIB_SHA1 WITH_LIB_SHA256 WITH_LIB_RC6 WITH_LIB_IDEA WITH_LIB_BIGNUM"
...
Examples:
  # Enable WEBIF and DVBAPI
  ./config.sh --enable WEBIF HAVE_DVBAPI

  # Disable WEBIF but enable DVBAPI
  ./config.sh --disable WEBIF --enable HAVE_DVBAPI
...
update_deps() {
	...
	enabled      MODULE_CCCAM        && enable_opts  WITH_LIB_RC6 WITH_LIB_SHA1 >/dev/null
	disabled     MODULE_CCCAM        && disable_opts WITH_LIB_RC6 WITH_LIB_SHA1 >/dev/null
	...
	enabled_any  READER_NAGRA_MERLIN WITH_SIGNING && enable_opt WITH_LIB_SHA256 >/dev/null
	disabled_all READER_NAGRA_MERLIN WITH_SIGNING && disable_opt WITH_LIB_SHA256 >/dev/null
	...
	enabled_any  READER_CONAX READER_CRYPTOWORKS READER_NAGRA READER_NAGRA_MERLIN       && enable_opt WITH_LIB_BIGNUM >/dev/null
	disabled_all READER_CONAX READER_CRYPTOWORKS READER_NAGRA READER_NAGRA_MERLIN       && disable_opt WITH_LIB_BIGNUM >/dev/null
	...
	enabled_any  MODULE_CAMD35 MODULE_CAMD35_TCP MODULE_CCCAM MODULE_MONITOR READER_NAGRA_MERLIN READER_VIACCESS READER_VIDEOGUARD                     && enable_opts  WITH_LIB_AES >/dev/null
	disabled_all MODULE_CAMD35 MODULE_CAMD35_TCP MODULE_CCCAM MODULE_MONITOR READER_NAGRA_MERLIN READER_VIACCESS READER_VIDEOGUARD                     && disable_opts WITH_LIB_AES >/dev/null
...
list_config() {
	...
	enabled_any MODULE_CAMD35 MODULE_CAMD35_TCP MODULE_CCCAM MODULE_MONITOR READER_NAGRA_MERLIN READER_VIACCESS READER_VIDEOGUARD && echo "CONFIG_WITH_LIB_AES=y" || echo "# CONFIG_WITH_LIB_AES=n"
	...
	enabled MODULE_CCCAM && echo "CONFIG_WITH_LIB_SHA1=y" || echo "# CONFIG_WITH_LIB_SHA1=n"
	...
	enabled_any READER_CONAX READER_CRYPTOWORKS READER_NAGRA READER_NAGRA_MERLIN && echo "CONFIG_WITH_LIB_BIGNUM=y" || echo "# CONFIG_WITH_LIB_BIGNUM=n"
	enabled READER_NAGRA_MERLIN && echo "CONFIG_WITH_LIB_MDC2=y" || echo "# CONFIG_WITH_LIB_MDC2=n"
	enabled_any READER_NAGRA_MERLIN WITH_SIGNING && echo "CONFIG_WITH_LIB_SHA256=y" || echo "# CONFIG_WITH_LIB_SHA256=n"
