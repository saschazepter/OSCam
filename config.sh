...
do
	...
	'--mbedtls-fetch')
		...
		#
		# The official tarballs bundle the tf-psa-crypto + framework subtrees
		# and all pre-generated source files, so the build needs no Python,
		# Perl, jsonschema, jinja2 or recursive submodule init — just the
		# plain POSIX toolchain (curl|wget, tar, sha256sum).
		...
		if [ -f "$STAMP" ]; then
			...
			echo "mbedTLS $VERSION ready (tarball ${sz:-?}, cached)"
			exit 0
		fi
		...
		echo "mbedTLS $VERSION ready (tarball $sz, fetched)"
		exit 0
	;;
	'-l'|'--list-config')
		list_config
