...
static int32_t connect_to_stream(char *http_buf, int32_t http_buf_len, char *stream_path, char *stream_source_host)
{
	...
	cs_log_dbg(D_CLIENT, "HTTP (send) (%i): %s", status, remove_newline_chars((const char *)&http_host));
...
static void *stream_client_handler(void *arg)
{
	...
	cs_log_dbg(D_CLIENT, "HTTP (recv) (%i): %s", clientStatus, remove_newline_chars((const char *)&http_host));
	...
	cs_log_dbg(D_CLIENT, "HTTP (send) (%i): %s", clientStatus, remove_newline_chars((const char *)&http_host));
